...
export default async function ToolsPage({ params, searchParams }: PageProps) {
  ...
  if (category) {
    toolsList = toolsList.filter(t => t.category_ar === category || t.category === category);
  }
    ...
    <main className="max-w-7xl mx-auto px-4 py-12 animate-fade-in-up">
      <div className="mb-10">
        ...
        <p className="text-sm text-slate-400 mt-1">
          {isAr ? تم العثور على \ أداة حقيقية محققة : Discovered \ verified production tools}
        </p>
      </div>

      {toolsList.length === 0 ? (
        <div className="text-center py-20 text-slate-400 border border-dashed rounded-2xl">
          <p className="text-base">{isAr ? 'لم نجد أي أداة تطابق خيارات البحث الحالية.' : 'No software matches your current filter criteria.'}</p>
        </div>
      ) : (
